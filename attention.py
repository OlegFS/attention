...
class RawData( AutoCR ):
    ...
    def __init__( self, prefix, keep_raw=False, copydict=None ):
        ...
        if copydict is not None:
            ...
            return
        # load data
        filenames = {'info': '%s/%s_trialinfo.mat' % (raw_data_dir, prefix),
             'rates': '%s/%s_ratesnex.mat' % (raw_data_dir, prefix),
             'times': '%s/%s_timesbytrialnex.mat' % (raw_data_dir, prefix)}
        ti = trialinfo = loadmat(filenames['info'])['trialinfo']
        r = rates = loadmat(filenames['rates'])
        t = times = loadmat(filenames['times'])
        # are we keeping the raw data 
        if keep_raw:
            self.raw_trialinfo = trialinfo
            self.raw_rates = Bunch(rates)
            self.raw__times = Bunch(times)
        ...
        self.trials = np.array([ 
            Trial( i, tt, 
                ...
                is_trial_instruction = self.is_trial_instruction[i],
                is_trial_valid = self.is_trial_valid[i],
                is_trial_catch = self.is_trial_catch[i],
                is_trial_cue_R = self.is_trial_cue_R[i],
                is_trial_target_R = self.is_trial_target_R[i],
                ...
                N_stimuli_presented = self.trial_N_stimuli_presented[i],
                target_orientation_change = self.target_orientation_change[i] )
