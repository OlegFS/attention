...
def sfft( x ):
    ...
    If multidimensional, always does this along the *last* axis. 

    """
    y = np.fft.rfft( x )
    N = shape( x )[-1]
    if is_even(N):
        return conc( [ y.real, y[..., 1:-1].imag ], axis=-1 )
    else:
        return conc( [ y.real, y[..., 1:].imag ], axis=-1 )

def isfft( y ):
    """ iFFT of the special representation above. 
    ...
    if y.dtype == complex:
        ...
        y2 = y[..., :N//2 + 1].astype(complex)
    if is_even(N):
        y2[..., 1:-1] += 1j * y[..., N//2 + 1:]
        return np.fft.irfft( y2 )
    else:
        y2[..., 1:] += 1j * y[..., N//2 + 1:]
        return np.fft.irfft( y2, n=N )

def sfreqs( N ):
    """ Absolute freqs in the sfft representation for a signal of length N. """
    freqs = np.linspace( 0, 0.5, N//2 + 1 )
    if is_even( N ):
        freqs = conc([ freqs, freqs[1:-1] ])
    else:
        freqs = conc([ freqs, freqs[1:] ])
    return freqs

def osfft( x ):
    """ Orthonormal version of special FFT. """
    N = shape(x)[-1]
    y = sfft( x ) * sqrt(2./N)
    y[..., 0] /= sqrt(2)
    if is_even( N ):
...
def iosfft( y ):
    """ Orthonormal version of special iFFT. """
    N = shape(y)[-1]
    y = y * sqrt(N/2.)
    y[..., 0] *= sqrt(2)
    if is_even( N ):
...
def rosfft( x, dims, **kw ):
    """ Reduced orthonormal special fft. Only specified dims are returned. """
    return osfft( x, **kw )[..., dims]

def irosfft( y, dims, **kw ):
    """ Reduced orthonormal special ifft. Only specified dims are provided. """
    # required size for reconstructed fft
    shap = list( shape(y) ) # req'd shape of the reconstructed fft
    N_sig = len(dims) # how long is the signal
    N_fft = N_sig//2 + 1 # how many complex coefficients in each recon fft
    shap[-1] = N_fft
    shap = tuple(shap)
    # initialise the fft
    y2 = zeros( shap, dtype=complex )
    # fill in the parts
    N_real = np.sum(dims[:N_fft])
    y2[..., dims[:N_fft]] = y[..., :N_real]
    if is_even( N_sig ):
        y2[..., 1:-1][..., dims[N_fft:]] += 1j * y[..., N_real:]
    else:
        y2[..., 1:][..., dims[N_fft:]] += 1j * y[..., N_real:]
    # the necessary rescaling
    y2[..., 0] *= sqrt(2)
    if is_even( N_sig ):
        y2[..., N_sig // 2] *= sqrt(2)
    # perform ifft
    x = np.fft.irfft(y2, n=N_sig, **kw) * sqrt(N_sig/2.)
    return x
    
def collapse_h( a, dims ):
    """ Reduce the dimensionality of `h`, along the *first* dim. """
    return rosfft( a.T, dims ).T

def expand_h( h_star, dims ):
    """ Expand the dimensionality of `h_star`, along the *first* dim. """
    return irosfft( h_star.T, dims ).T

def calc_abs_freqs( T, pad_size ):
    """ Absolute frequencies of the special FFT.
    
    Assumes that the real fft is split up into real components, and *then*
    imaginary components.

    Depends on whether T is even or odd.
    
    """
    T = T + 2 * pad_size
    iseven = (T == int(T // 2) * 2)
    if iseven:
        abs_freqs = np.linspace(0, 0.5, T//2 + 1)
        abs_freqs = conc([ abs_freqs, abs_freqs[1:-1] ])
    else:
        abs_freqs = np.linspace(0, 0.5, T//2 + 1)
        abs_freqs = conc([ abs_freqs, abs_freqs[1:] ])
    return abs_freqs
...
class Data( AutoReloader ):
    ...
    def __init__( self, X, y, 
        ...
        Provide the stimulus `X`, which is size ( T x D ).
        Provide the response `y`, which is size ( T ).

        Optional keywords:

        - `normalise` : transform each of the D dimensions of `X` to have
            zero mean and unitary standard deviation

        - `whiten` : transform so that each of the D dimensions of `X` has
            zero mean, and X^T.X = I

        - `add_constant` : append an extra dimension to X which is constant 
        ...
        if normalise or whiten:
            ...
            self._Xs = Xs = np.std( X, axis=0 )
            assert np.min(Xs) > 0
            X = ( X - Xm[na, :] ) / Xs[na, :]
        if whiten:
            self.announce('whitening', prefix='Data')
            self._L, self._Q = L, Q = eigh(dot(X.T, X))
        ...
        self.X, self.y = X, y
        # sizes
        T, D = self.T, self.D = shape( self.X )
        assert len(y) == self.T
    ...
    def plot_h( self, posterior_h, n_samples=10, 
            exponentiate=True, remove_offset=True, show_pad=False, 
            testing_slices=None, training_slices=None, linear_interpolate=True,
            draw=True, new_figure=True, ax=None, return_ax=False, 
            return_fig=False, title=None, **kw ):
        """ Plot the estimated log modulator (h) over time.
        
        This will also show samples from the estimated posterior.

        Must provide:

        - `posterior_h` : Posterior on `h`. 
        ...
        - `exponentiate`: if False, plot `h`, the log of the latent
            modulator. If True, plot `g`, the latent modulator.

        - `remove_offset`: if True, normalise by removing the DC term
            of `h`.

        - `show_pad`: if True, zoom out to show the padding of `h`.

        Displaying training / testing regions for cross-validations:

        - `testing_slices` / `training_slices`: list of regions of the data 
            which are testing / training regions, respectively. These are 
            coloured differently. If either is None or an empty list, then no 
            separate regions are shown.

        - `linear_interpolate` : boolean. Whether to linearly interpolate the
            log gain signal, rather than rely on the smooth (Fourier)
            interpolation of `h`.
        ...
        i = np.arange(T)
        if not exponentiate:
            ax.plot( i, zeros(T), 'k-', alpha=0.2 )
        else:
            ax.plot( i, ones(T), 'k-', alpha=0.2 )
        # are we exponentiating
        if not exponentiate:
            def f(x):
                return x
        else:
            f = exp
        # parse slices
        if testing_slices is None or training_slices is None:
            testing_slices = []
            training_slices = [slice(None)]
            plot_slices = False
        else:
            plot_slices = True
        # calculate mean estimate (with baseline correction)
        h_hat = posterior_h.h
        offset = ( np.mean(h_hat) if remove_offset else 0 )
        h_hat = h_hat - offset
        # are we interpolating
        if plot_slices and linear_interpolate:
            h_hat_original = h_hat.copy()
            for s in testing_slices:
                if s.start == 0:
                    h_hat[s] = h_hat[s.stop]
                elif s.stop == self.T:
                    h_hat[s] = h_hat[s.start]
                else:
                    h_hat[s] = np.linspace(
                            h_hat[s.start], h_hat[s.stop], s.stop - s.start )
            h_correction = h_hat - h_hat_original
        else:
            h_correction = np.zeros_like( h_hat )

        # plot samples from the distribution over `h`
        if n_samples > 0:
            h_star = posterior_h.h_star
            samples_star = multivariate_normal(
                    h_star, posterior_h.Lambda_h_star, size=n_samples )
            dims_h = posterior_h.dims_h
            samples = expand_h( samples_star.T, dims_h ).T - offset
            samples[:, :self.T] += h_correction[na, :]
            # plot samples 
            tt = np.arange( shape(samples)[1] )
            sample_kws = {'alpha':10./n_samples}
            if not plot_slices:
                ax.plot( tt, f(samples.T), 'r', **sample_kws )
            else:
                for s in training_slices:
                    ax.plot( tt[s], f(samples.T[s, :]), 'r-', **sample_kws )
                for s in testing_slices:
                    ax.plot( tt[s], f(samples.T[s, :]), 'y-', **sample_kws )

        # plot mean estimate of `h`
        if not plot_slices:
            ax.plot( i, f(h_hat), 'k-', lw=2 )
        else:
            for s in training_slices:
                ax.plot( i[s], f(h_hat[s]), 'k-', lw=2 )
            for s in testing_slices:
                ax.plot( i[s], f(h_hat[s]), '-', lw=2, color=[0.5]*3 )
        ...
        if show_pad:
            ax.set_xlim( 0, shape(samples)[1] - 1 )
            ax.set_xticks([0, T, shape(samples)[1]])
            if hasattr( self, 'N_sec' ):
                ax.set_xticklabels([0, self.N_sec, 2*self.N_sec])
        else:
            ax.set_xlim(0, T - 1)
            ax.set_xticks([0, T])
            if hasattr( self, 'N_sec' ):
                ax.set_xticklabels([0, self.N_sec])
        if hasattr( self, 'N_sec' ):
